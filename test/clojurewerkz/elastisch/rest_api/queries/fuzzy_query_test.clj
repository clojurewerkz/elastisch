...
(deftest ^{:query true} test-basic-fuzzy-query-with-string-fields
  (let [response (doc/search index-name mapping-type :query (q/fuzzy :title "Nueva"))
...
(deftest ^{:query true} test-basic-fuzzy-query-with-numeric-fields
  (let [response (doc/search index-name mapping-type :query (q/fuzzy :number-of-edits {:value 13000 :min_similarity 3}))
        hits     (hits-from response)]
    (is (any-hits? response))
    (is (= 1 (total-hits response)))
...
#_ (deftest ^{:query true} test-basic-fuzzy-query-with-date-fields
     (let [response (doc/search index-name mapping-type :query (q/fuzzy "last-edit.date" {:value "2012-03-25T12:00:00" :min_similarity "1d"}))
           hits     (hits-from response)]
       (is (any-hits? response))
       (is (= 1 (total-hits response)))
       (is (= #{"4"} (ids-from response)))))
